...
$input = '<input type="text" name="q" id="mod-finder-searchword' . $module->id . '" class="js-finder-search-query uk-search-input" value="' . htmlspecialchars($app->getInput()->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
    . ' placeholder="' . Text::_('MOD_FINDER_SEARCH_VALUE') . '">';

$showLabel  = $params->get('show_label', 1);
$labelClass = (!$showLabel ? 'visually-hidden ' : '') . 'uk-hidden finder';
...
if ($params->get('show_button', 0)) {
    ...
    $output .= $input;
    $output .= '<button uk-search-icon class="uk-search-icon-flip uk-icon uk-search-icon" type="submit" aria-label="' . Text::_('JSEARCH_FILTER_SUBMIT') . '"></button>';
    $output .= '</div>';
} else {
    $output .= '<span uk-search-icon class="uk-position-z-index"></span>';
    $output .= $label;
    $output .= $input;
}
...
<search>
    <form class="mod-finder js-finder-searchform uk-search uk-search-default uk-width-1-1" action="<?php echo Route::_($route); ?>" method="get" aria-label="search">
        <?php echo $output; ?>
