	...
	static function getList(&$params, $menu_type = false)
	{
		...
		if ($items) {
			...
			foreach ($items as $i => $item) {
				...
				if (!$item->getParams()->get('menu_show', 1) && strpos($item->getParams()->get('menu-anchor_css', ''), 's5_logo_spacer') === false) {
					unset($items[$i]);
					continue;
				}
...
//						}
				...
				$item->anchor_css = htmlspecialchars($item->getParams()->get('menu-anchor_css', ''));

				//$item->anchor_title = htmlspecialchars($item->params->get('menu-anchor_title', ''));
				$item->anchor_title = htmlspecialchars($item->getParams()->get('menu-anchor_title', ''));

				//$item->menu_image = $item->params->get('menu_image', '') ? htmlspecialchars($item->params->get('menu_image', '')) : '';
				$item->menu_image = $item->getParams()->get('menu_image', '') ? htmlspecialchars($item->getParams()->get('menu_image', '')) : '';
	...
	public static function S5buildXML($params, $menu_type = false, $version = false)
	{
		...
		$Itemid = (!$Itemid) ? Joomla\CMS\Factory::getApplication()->getInput()->get('Itemid') : intval($Itemid);
		...
		if (is_array($rows)) {
			...
			if (!empty($parent_items)) {
				...
				foreach ($parent_items as $prow => $pitem) {
					...
					$parent_subtext_flex = "";
					$parent_attr_title = $pitem->anchor_title ? 'title="' . $pitem->anchor_title . '"' : '';
					...
					//	$S5_menu_items_params->loadJSON($pitem->getParams());

					//else
						//$S5_menu_items_params->loadObject($pitem->params);
						$S5_menu_items_params->loadObject($pitem->getParams());
				...
				// print_r($pitem->anchor_title);die;
					...
					//	$iParams->loadJSON($pitem->getParams());

					//else
						//$iParams->loadObject($pitem->params);
						$iParams->loadObject($pitem->getParams());
					...
					*/
					...
					switch ($pitem->browserNav) {
							...
							if (strstr($pitem->url, "#s5")) {
								...
								if ($pitem->title != "") {
									...
									if (strrpos($pitem->url, "/") > 1) {
										...
										$link_format = "<a " . $parent_attr_title . " href=\"$pitem->url\"' onclick=\"s5_page_scroll('" . $pitem->url . "')\"><span onclick=\"s5_page_scroll('" . $pitem->url . "')\" >" . $pitem->title . "</span></a>";
								...
								if ($pitem->title != "") {
									...
									$link_format = "<a " . $parent_attr_title . " href=\"$pitem->url\"><span onclick='window.document.location.href=\"$pitem->url\"'>" . $pitem->title . "</span></a>";
						...
						case 1:
							...
							$link_format = "<a  " . $parent_attr_title. " href=\"$pitem->url\" onclick='this.removeAttribute(\"href\")' target='_blank'><span onclick='window.open(\"$pitem->url\")'>" . $pitem->title . "</span></a>";
						...
						case 2:
							...
							$link_format = '<a  ' . $parent_attr_title . ' href="' . $link . '" onclick="this.removeAttribute(\'href\')"><span onclick="window.open("' . $link . '",\'targetWindow\',\'' . $attribs . '\');return false;">' . $pitem->title . '</span></a>';
					...
					$display .= S5modMainMenuHelper::getChildItems($prow, $child_items, $maxdepth, 2, $S5_child_columns, $pitem->getParams());
			...
			if (isset($child_items[$current_depth][$parent_id]) && !empty($child_items[$current_depth][$parent_id])) {
				...
				if ($child_records_per_row > 0) {
					...
					foreach ($child_items[$current_depth][$parent_id] as $crow => $citem) {
						...
						//	$S5_menu_items_params->loadJSON($citem->getParams());
						//else
							//$S5_menu_items_params->loadObject($citem->params);
							$S5_menu_items_params->loadObject($citem->getParams());
						$S5_child_columns = $S5_menu_items_params->get(($flexmv <= 2.5 ? 's5_columns' : 'data.s5_columns'), 1); //s5_child_columns
						...
						if ($S5_load_mod == 0) {
							if (isset($child_items[$current_depth + 1][$crow])) {
								...
								$display_c .= S5modMainMenuHelper::getChildItems($crow, $child_items, $maxdepth, $current_depth + 1, $S5_child_columns, $citem->getParams());
							}
						}
						//display the child content grouped or not
						if ($s5_group_child == 0) {
	...
	public static function getMenuContent($S5row)
	{
		...
		$attr_title = ($S5row->anchor_title) ? "title=\"$S5row->anchor_title\"" : "";
		...
		//	$S5_menu_items_params->loadJSON($S5row->getParams());

		//else
			//$S5_menu_items_params->loadObject($S5row->params);
			$S5_menu_items_params->loadObject($S5row->getParams());
		...
		//	$iParams->loadJSON($tmp->getParams());

		//else
			//$iParams->loadObject($tmp->params);
			$iParams->loadObject($tmp->getParams());
		...
		*/
		...
		switch ($tmp->browserNav) {
				...
				if (strstr($S5row->url, "#s5")) {
					...
					if ($S5row->title != "") {
						...
						if (strrpos($S5row->url, "/") > 1) {
							...
							$link_format = "<a $attr_title href=\"$S5row->url\" onclick=\"s5_page_scroll('" . $S5row->url . "')\"><span class='s5_sub_a_span'  onclick=\"s5_page_scroll('" . $S5row->url . "')\" >" . $S5row->title . "</span></a>";
					...
					$link_format = "<a $attr_title href=\"$S5row->url\"><span class='s5_sub_a_span' onclick='window.document.location.href=\"$S5row->url\"'>" . $S5row->title . "</span></a>";
			...
			case 1:
				...
				$link_format = "<a $attr_title href=\"$S5row->url\" target='_blank' onclick='this.removeAttribute(\"href\")'><span class='s5_sub_a_span' onclick='window.open(\"$S5row->url\")'>" . $S5row->title . "</span></a>";
			...
			case 2:
				...
				$link_format = '<a '.$attr_title.' href="' . $link . '" onclick="this.removeAttribute(\'href\')"><span class="s5_sub_a_span" onclick="window.open(\'' . $link . '\',\'targetWindow\',\'' . $attribs . '\');return false;">' . $S5row->title . '</span></a>';
