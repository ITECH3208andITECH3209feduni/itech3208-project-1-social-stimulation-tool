...
# @version $Id: jsn_readxmlfile.php 15847 2012-09-06 03:00:46Z tuyetvt $
-------------------------------------------------------------------------*/
...
class JSNReadXMLFile
{
	...
	{
		$filePath = $this->_template_folder_path.DS.'templateDetails.xml';
		...
		$data->edition 		= $xml->edition ? (string) $xml->edition : '';
 		$data->license 		= $xml->license ? (string) $xml->license->data() : '';
 		$data->description 	= $xml->description ? (string) $xml->description->data() : '';
	...
	function getExtensionManifestFileInformation($name, $manifestPath)
	{
		...
		$filePath = JPATH_ROOT.DS.str_replace('/', DS, $manifestPath);
		if ($name == 'imageshow')
		{
			if (!JFile::exists($filePath))
			{
				$filePath = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_'.$name.DS.'com_'.$name.'.xml';
			}
		}
	...
	function getSampleDataFileContent($path, $templateName, $onlyExtInfo = false)
	{
		...
		$path = JPath::clean($path.DS.$xml_file);
